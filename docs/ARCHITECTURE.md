...
- 多個倒數可同時執行，並可逐筆操作。
- 預設及自訂倒數可從 20 種具名識別色中選擇。
- 首頁活動倒數卡片使用計時器的 `colorValue` 作為實心背景，並以色彩亮度選擇黑色或白色前景。
...
- 鈴響時間的已選狀態使用 Material 主色、`onPrimary` 高對比內容、粗體文字及勾選圖示；主題色使用 Material 預設 FilterChip 選取樣式。
